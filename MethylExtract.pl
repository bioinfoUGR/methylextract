...
my ($inDir,$seq,$context,$checkBisulfite,$methNonCpGs,$qscore,$Q,$minSNVperc,$outDir,$p,$delDup,$FirstIgnor,$tagW,$tagC,$simDupPb,$chromDiv,$memNumReads,$chromSort,$minDepthSNV,$maxPval,$minDepthMeth,$maxStrandBias,$typeFasta,$multiSeqID,$peOverlap,$LastIgnor);
...
($inDir,$seq,$context,$checkBisulfite,$methNonCpGs,$qscore,$Q,$minSNVperc,$outDir,$bedOut,$wigOut,$p,$delDup,$FirstIgnor,$tagW,$tagC,$vcfOut,$simDupPb,$chromDiv,$memNumReads,$chromSort,$minDepthSNV,$maxPval,$minDepthMeth,$maxStrandBias,$typeFasta,$multiSeqID,$peOverlap,$LastIgnor)=
...
if ($bedOut eq "Y") {
	if ($context eq "CG" or $context eq "ALL") {
		...
		print BEDOUT "track name=CG Methylation  description=MethylExtract_1.8 visibility=1 useScore=1\n";
		for (my $j=0;$j<=$#chroms;$j++) {
	...
	if ($context eq "CHG" or $context eq "ALL") {
		...
		print BEDOUT "track name=CHG Methylation  description=MethylExtract_1.8 visibility=1 useScore=1\n";
		for (my $j=0;$j<=$#chroms;$j++) {
	...
	if ($context eq "CHH" or $context eq "ALL") {
		...
		print BEDOUT "track name=CHH Methylation  description=MethylExtract_1.8 visibility=1 useScore=1\n";
		for (my $j=0;$j<=$#chroms;$j++) {
...
if ($wigOut eq "Y") {
	if ($context eq "CG" or $context eq "ALL") {
		...
		print WIGOUT "track type=wiggle_0 name=CG Methylation  description=MethylExtract_1.8 glyph=wiggle_density bgcolor=red fgcolor=red\n";
		for (my $j=0;$j<=$#chroms;$j++) {
	...
	if ($context eq "CHG" or $context eq "ALL") {
		...
		print WIGOUT "track type=wiggle_0 name=CHG Methylation  description=MethylExtract_1.8 glyph=wiggle_density bgcolor=red fgcolor=red\n";
		for (my $j=0;$j<=$#chroms;$j++) {
	...
	if ($context eq "CHH" or $context eq "ALL") {
		...
		print WIGOUT "track type=wiggle_0 name=CHH Methylation  description=MethylExtract_1.8 glyph=wiggle_density bgcolor=red fgcolor=red\n";
		for (my $j=0;$j<=$#chroms;$j++) {
...
return($inDir,$seq,$_[0],$_[1],$_[2],$_[3],$_[4],$_[5],$_[6],$_[7],$_[8],$_[9],$_[10],$_[11],$_[12],$_[13],$_[14],$_[15],$_[16],$_[17],$_[18],$_[19],$_[20],$_[21],$_[22],$_[23],$_[24],$_[25],$_[26]);
}
